...
class Connection extends EventEmitter {
    constructor(address) {
        ...
        this.socket.on('listening', () => {
            ...
            console.log(`Socket server is listening on ${socketAddress.address}:${socketAddress.port}`)
    ...
    scan(networks) {
        ...
        networks.split(';').forEach((networkAddress) => {
            console.log(`Scanning network ${networkAddress} for available devices...`)
    ...
    async sendRequest(address, port, key, payload) {
        return new Promise((resolve, reject) => {
            ...
            const messageHandler = (msg, rinfo) => {
                ...
                console.log(`Received message from ${message.cid} (${rinfo.address}:${rinfo.port}) ${msg.toString()}`);
            ...
            console.log(`Sending request to ${address}:${port}: ${JSON.stringify(payload)}`);
